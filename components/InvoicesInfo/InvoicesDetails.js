...
import {
  ...
  Loader2, FileText, User, Pencil, Save, Lock
} from "lucide-react";
...
export default function InvoicesDetails() {
  ...
  // Note: In separated files, useEffect dependencies must be present.
  // If you have this in the main file, ensure editFormData is properly initialized.
  const [localEditFormData, setLocalEditFormData] = useState({}); 
  ...
  // Pre-fill edit form when data changes
  if (showEditModal && feeBreakdown && Object.keys(editFormData).length === 0) {
      const initialData = {};
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
              <div className="flex items-center justify-between px-6 py-4 border-b bg-amber-50 rounded-t-2xl">
                ...
                <button onClick={() => { setShowEditModal(false); setSelectedInvoice(null); setEditFormData({}); }} className="p-2 rounded-full hover:bg-amber-100 text-amber-800 transition"><XCircle size={24} /></button>
              </div>
